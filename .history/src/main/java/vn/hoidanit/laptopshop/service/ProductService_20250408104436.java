...
public class ProductService {
    ...
    public Page<Product> searchProductsWithPagination(String keyword, Pageable pageable) {
        ...
        for (String term : terms) {
            // Check if term contains price information
            if (term.contains("giá")) {
                // Extract price range
                for (int i = 0; i < terms.length - 1; i++) {
                    if (terms[i].equals("từ") && i + 3 < terms.length && terms[i + 2].equals("đến")) {
                        try {
                            minPrice = Double.parseDouble(terms[i + 1]) * 1000000; // Convert to VND
                            maxPrice = Double.parseDouble(terms[i + 3]) * 1000000;
                            i += 3; // Skip processed terms
                        } catch (NumberFormatException e) {
                            // Invalid number format, continue with normal search
                        }
                    }
                }
            } else if (!term.equals("từ") && !term.equals("đến") && !term.equals("triệu")) {
                searchTerms.add(term);
            }
        }

        // First term is usually the manufacturer
        if (!searchTerms.isEmpty()) {
            searchFactory = searchTerms.get(0);
            if (searchTerms.size() > 1) {
                // Combine remaining terms for target
                searchTarget = String.join(" ", searchTerms.subList(1, searchTerms.size()));
            }
        }
