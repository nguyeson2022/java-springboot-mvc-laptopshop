...
public class ProductService {
    ...
    public Page<Product> searchProductsWithPagination(String keyword, Pageable pageable) {
        ...
        if (keyword != null && !keyword.trim().isEmpty()) {
            ...
            for (int i = 0; i < words.length; i++) {
                if (words[i].equals("giá") && i + 4 < words.length) {
                    if (words[i + 1].equals("từ") && words[i + 3].equals("đến")) {
                        try {
                            minPrice = (long) (Double.parseDouble(words[i + 2]) * 1000000);
                            maxPrice = (long) (Double.parseDouble(words[i + 4]) * 1000000);
                            i += 4; // Skip processed price-related terms
                            continue;
                        } catch (NumberFormatException e) {
                            // If price parsing fails, treat as normal search terms
                        }
                    }
                }
                // Add non-price terms to search
                if (!words[i].equals("triệu")) {
                    searchTerms.append(words[i]).append(" ");
                }
            }

            searchKeyword = searchTerms.toString().trim();
            if (searchKeyword.isEmpty()) {
                searchKeyword = null;
            }
        }

        return productRepository.searchProducts(searchKeyword, minPrice, maxPrice, pageable);
    }
    ...
    public List<Product> getRelatedProducts(Product product, int limit) {
        // Find products with same factory or target, excluding current product
        Page<Product> relatedProducts = productRepository.findByFactoryOrTargetAndIdNot(
                product.getFactory(),
                product.getTarget(),
                product.getId(),
    ...
    public Map<String, Long> getFactoryCounts() {
