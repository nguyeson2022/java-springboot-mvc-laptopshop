...
public class ProductService {
    ...
    public Page<Product> searchProductsWithPagination(String keyword, Pageable pageable) {
        ...
        // Handle case where search has two parts (e.g. "asus sinh-viên" or "asus văn
        // phòng")
        if (terms.length >= 2) {
            searchFactory = terms[0]; // First term assumed to be manufacturer
            // Combine remaining terms for target
            searchTarget = String.join(" ", Arrays.copyOfRange(terms, 1, terms.length))
                    .replace("-", " "); // Replace hyphens with spaces
        }

        return productRepository.searchProducts(keyword, searchFactory, searchTarget, pageable);
    }
}
